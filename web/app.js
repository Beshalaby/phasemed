...
const state = {
  ...
  volume: null, slice: 0, windowCenter: 40, windowWidth: 400, timelineValue: 100, toastTimer: null, previewToken: 0,
...
function renderInspector() {
  ...
  const extent = dims.length ? Math.max(...dims) : null; const change = object.temporal_links?.[0]?.changes?.volume_change_percent;
...
function drawSceneOn(canvas, compact = false) {
  ...
  const all = objects().filter((item) => geometryOf(item)); if (!all.length) return;
  const bounds = boundsFor(all); const angle = state.mode === "hologram" ? .28 : state.sceneAngle; const visible = [...all].sort((a, b) => (a.type === "volume" ? -1 : 0) - (b.type === "volume" ? -1 : 0));
  ...
  const drawObject = (item, ghost = false) => { if (state.isolate && item.id !== state.selectedId) return; const center = pointFor(item); const size = projectSize(item, bounds, width, height); const selected = item.id === state.selectedId; const color = typeColor(item); ctx.save(); ctx.translate(center[0], center[1]); ctx.globalAlpha = ghost ? .18 : selected ? 1 : item.type === "volume" ? .18 : .72; ctx.fillStyle = color; ctx.strokeStyle = color;
  ...
  if (state.timelineValue < 50 && state.model?.temporal_links?.some((link) => link.source_object_id === state.selectedId)) drawObject(currentObject(), true);
  visible.forEach((item) => drawObject(item));
...
function renderTimeline() { const model = state.model; const timeline = model?.timeline || []; $("timelineRail").innerHTML = timeline.length ? timeline.map((entry, index) => `<div class="timeline-node ${index < timeline.length - 1 ? "prior" : ""}" style="left:${timeline.length === 1 ? 50 : (index / (timeline.length - 1)) * 100}%"><div class="timeline-node-label"><strong>${escapeHtml(entry.label || entry.study_id)}</strong>${escapeHtml(entry.date || "")}</div></div>`).join("") : ""; const hasPrior = Boolean(model?.metadata?.prior_model_id || (model?.temporal_links || []).length); $("timelineStatus").textContent = hasPrior ? `${model.temporal_links?.length || 0} object links` : ""; const links = model?.temporal_links || []; $("timelineDetail").innerHTML = links.length ? `<div class="change-grid"><div><span>Linked</span><strong>${links.filter((link) => link.type === "same_as_prior").length}</strong></div><div><span>Changed</span><strong>${links.filter((link) => link.type === "changed_from").length}</strong></div><div><span>New</span><strong>${links.filter((link) => link.type === "new").length}</strong></div><div><span>Resolved</span><strong>${links.filter((link) => link.type === "resolved").length}</strong></div></div>` : `<span class="timeline-empty">No comparison available.</span>`; }
...
async function applyTimeline(value) { const nextPrior = Number(value) < 50; if (nextPrior && state.priorModel && state.viewModel?.id !== state.priorModel.id) { state.viewModel = state.priorModel; state.selectedId = state.model?.temporal_links?.find((link) => link.source_object_id === state.selectedId)?.target_object_id || defaultObjectId(state.priorModel); await loadVolume(state.priorStudy); } else if (!nextPrior && state.model && state.viewModel?.id !== state.model.id) { const priorId = state.selectedId; state.viewModel = state.model; state.selectedId = state.model.temporal_links?.find((link) => link.target_object_id === priorId)?.source_object_id || defaultObjectId(state.model); await loadVolume(state.study); } renderInspector(); drawAll(); updateCrosshair(); }
...
function wireEvents() {
  ...
  $("resetButton").addEventListener("click", () => { state.isolate = false; state.showLinks = true; state.pathMode = false; state.pathPoints = []; state.measurePoints = []; state.measureResult = null; state.sceneAngle = .12; state.sceneZoom = 1; state.timelineValue = 100; $("timelineInput").value = "100"; $("measureReadout").classList.add("hidden"); state.viewModel = state.model; loadVolume(state.study); drawAll(); renderInspector(); toast("View reset"); });
