...
function renderRailObjects() {
  ...
  list.innerHTML = items.length ? items.map((item) => `<button class="rail-row ${item.id === state.selectedId ? "active" : ""} ${isHighlighted(item) ? "flagged" : ""}" data-rail-object="${escapeHtml(item.id)}"><i style="background:${typeColor(item)}"></i><span><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.type)}</small></span><em>${isHighlighted(item) ? "flagged" : "›"}</em></button>`).join("") : `<div class="rail-placeholder">Open a compiled model to browse objects.</div>`;
...
function renderChat() {
  ...
  thread.innerHTML = messages.length ? messages.map((message, index) => {
    ...
    const flags = flagged.length ? `<div class="chat-flags"><small>Flagged</small>${flagged.map((object) => `<button type="button" class="chat-chip flagged" data-chat-object="${escapeHtml(object.id)}"><i></i>${escapeHtml(object.label)}</button>`).join("")}<button type="button" class="chat-clear" data-chat-clear>Clear</button></div>` : "";
