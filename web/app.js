...
function renderAnalysisPanels() {
  const object = currentObject(); const graph = $("graphInspector");
  if (!object) { graph.innerHTML = `<div class="rail-placeholder">Select an object to inspect its spatial graph.</div>`; return; }
...
function wireEvents() {
  ...
  $("capabilityInfoLibrary").addEventListener("click", () => showSheet("capabilitySheet")); $("aiForm").addEventListener("submit", (event) => { event.preventDefault(); const question = $("aiInput").value; $("aiInput").value = ""; askModel(question); }); $("aiThread").addEventListener("click", () => {}); document.querySelectorAll("[data-query]").forEach((button) => button.addEventListener("click", () => askModel(button.dataset.query)));
