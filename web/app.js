...
const state = {
  ...
  chat: { messages: [], busy: false, abort: null, modelVersion: null, capability: null },
...
function openHologramTab({ announce = true } = {}) {
  ...
  if (!display) { if (announce) toast("The browser blocked the hologram tab · allow pop-ups for Phasmed"); return null; }
...
// Words for the enums the API speaks in. Anything unlisted falls back to the enum with its underscores removed.
const STATUS_LABELS = { ready: "Model ready", imported: "Needs a model", compiling: "Building", failed: "Build failed", unreviewed: "Not reviewed", confirmed: "Confirmed", rejected: "Rejected", changed_from: "Changed", same_as_prior: "Unchanged", new: "New", new_since_prior: "New", resolved: "Resolved", resolved_since_prior: "Resolved", available: "Available", partial: "Partial", configured: "Configured", demo_fixture: "Demo data", not_configured: "Not set up", unavailable: "Unavailable", dicomweb: "Hospital image server (DICOMweb)", local_dicom: "Local DICOM folder", c_store: "Scanner push (C-STORE)", qido_rs: "Study search (QIDO-RS)", wado_rs: "Image retrieval (WADO-RS)", stow_rs: "Image upload (STOW-RS)" };
function humanStatus(value) { const key = String(value ?? ""); return STATUS_LABELS[key] || key.replaceAll("_", " "); }
function formatStudyDate(value) { const match = /^(\d{4})-?(\d{2})-?(\d{2})/.exec(String(value || "")); if (!match) return String(value || ""); return `${Number(match[3])} ${["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][Number(match[2]) - 1]} ${match[1]}`; }
function shortStudyTitle(text) { return String(text || "").split(" (")[0]; }
...
// "" means the user cleared the selection on purpose; null means nothing chosen yet, so fall back to a default.
function currentObject() { if (state.selectedId === "") return null; return objects().find((item) => item.id === state.selectedId) || objects()[0] || null; }
...
// Segmenter labels are identifiers ("lung_upper_lobe_left"); show people a name and keep the id for matching.
const LABEL_WORDS = { vertebrae: "vertebra", clavicula: "clavicle", autochthon: "deep back muscle" };
function displayLabel(label) {
  const raw = String(label || "");
  if (!/^[a-z0-9]+(_[a-zA-Z0-9]+)+$/.test(raw) && !/^[a-z]+$/.test(raw)) return raw;
  let parts = raw.split("_");
  const side = parts.find((part) => part === "left" || part === "right");
  parts = parts.filter((part) => part !== side).map((part) => LABEL_WORDS[part] || part);
  let name;
  const lobe = parts.indexOf("lobe");
  if (parts[0] === "lung" && lobe > 0) name = `lung · ${parts.slice(1, lobe).join(" ")} lobe`;
  else name = parts.join(" ");
  const text = side ? `${side} ${name}` : name;
...
function renderStudies() { $("demoCredit")?.classList.toggle("hidden", !state.studies.some((study) => String(study.patient_id || "").startsWith("DEMO-")));
  ...
  list.innerHTML = visible.map((study) => {
    ...
    const detail = study.status === "ready" ? `${study.image_count || 0} images · ${study.series_count || 0} series` : study.status === "compiling" ? "Patient model is building" : `${study.image_count || 0} images · no model yet`;
    ...
    return `<article class="study-card ${active ? "active" : ""}" data-study-id="${escapeHtml(study.id)}" tabindex="0"><div class="study-preview">${preview ? `<img src="${preview}" alt="" loading="lazy" />` : `<span>${escapeHtml(study.modality || "DCM")}</span>`}<i class="study-dot ${statusClass}"></i></div><div class="study-card-body"><div class="study-card-top"><span>${escapeHtml(formatStudyDate(study.study_date) || "Undated")}</span><b>${escapeHtml(study.modality || "DICOM")}</b></div><h3>${escapeHtml(study.description || "Imported study")}</h3><p>${escapeHtml(detail)}</p><div class="study-card-bottom"><span>${escapeHtml(humanStatus(study.status))}</span>${study.status === "imported" ? `<button class="text-button" data-compile-study="${escapeHtml(study.id)}">Build model</button>` : `<span>${active ? "OPEN" : "OPEN →"}</span>`}</div></div></article>`;
  }).join("");
...
function renderInspector() {
  const object = currentObject(); $("inspectorTitle").textContent = displayLabel(object?.label) || "Nothing selected";
  if (!object) { $("inspectorContent").innerHTML = `<div class="inspector-empty"><span class="empty-dot"></span><p>Click a structure in the model to inspect it. Click it again to clear the selection.</p></div>`; return; }
  ...
  $("askModelInline").addEventListener("click", () => openChat());
...
// Each secondary canvas keeps its own camera, so panning the model tab cannot empty the
// review overview, and the camera a canvas was drawn with is remembered for click mapping.
const newView = () => ({ yaw: CAM_DEFAULT.yaw, pitch: CAM_DEFAULT.pitch, zoom: 1, panX: 0, panY: 0 });
const canvasViews = { mprModelCanvas: newView(), procedureCanvas: newView() };
const canvasCams = new WeakMap();
function resetCanvasViews() { Object.keys(canvasViews).forEach((id) => Object.assign(canvasViews[id], newView())); }
function drawSceneOn(canvas, compact = false) {
  ...
  const bounds = sceneBounds(all); const view = canvasViews[canvas.id];
  const cam = view ? camFor(bounds, width, height, view.yaw, view.pitch, view.zoom, view.panX, view.panY) : camFor(bounds, width, height);
  canvasCams.set(canvas, cam);
  ...
  if (!compact && selected) { const point = pointFor(selected); const callout = $("sceneCallout"); callout.classList.remove("hidden"); callout.style.left = `${clamp(point[0] + 18, 20, width - 190)}px`; callout.style.top = `${clamp(point[1] - 20, 76, height - 76)}px`; $("calloutTitle").textContent = displayLabel(selected.label); $("calloutText").textContent = `${selected.type} · click to inspect`; } else $("sceneCallout").classList.add("hidden");
  return { bounds, width, height, cam };
}
function updateOrientation(cam) { const widget = $("orientWidget"); if (!widget || !cam) return; const axes = { L: [1, 0, 0], R: [-1, 0, 0], P: [0, 1, 0], A: [0, -1, 0], S: [0, 0, 1], I: [0, 0, -1] }; widget.querySelectorAll("[data-axis]").forEach((label) => { const axis = axes[label.dataset.axis]; const x = dot3(cam.R, axis), y = dot3(cam.U, axis), toward = dot3(cam.T, axis); label.style.left = `${32 + 26 * x}px`; label.style.top = `${32 - 26 * y}px`; label.style.opacity = Math.hypot(x, y) < .25 ? 0 : .35 + .65 * (.5 + .5 * toward); }); const bar = document.querySelector(".model-scale"); if (bar) { const length = [100, 50, 20, 10, 5].find((mm) => mm * cam.s <= 120) || 5; bar.querySelector("span").style.width = `${Math.round(length * cam.s)}px`; bar.querySelector("b").textContent = `${length} mm`; } }
function drawScene() { const result = drawSceneOn($("sceneCanvas")); updateOrientation(result?.cam); const preset = anatomyPreset(); $("sceneSummary").textContent = preset && state.scenePreset === preset.id && !state.isolate ? `${preset.label} · segmented anatomy` : displayLabel(currentObject()?.label) || "Select an object to inspect it"; const loading = window.MeshView?.pending?.() || 0; const broken = window.MeshView?.failed?.() || 0; $("sceneLoad").textContent = loading ? `Loading anatomy · ${loading}` : broken ? `${broken} ${broken === 1 ? "structure" : "structures"} could not be drawn` : ""; $("sceneLoad").classList.toggle("warn", !loading && broken > 0); updateSceneControls(); return result; }
...
async function sendVoiceClip(blob) {
  ...
  try {
    ...
    const answered = await sendChat(transcript); showHologramCaption(transcript, state.chat.messages.at(-1));
...
function gestureScanPreviewUrl() {
  ...
  return `/api/studies/${encodeURIComponent(study.id)}/series/${encodeURIComponent(series)}/mpr?plane=axial&index=${encodeURIComponent(state.slice)}&window_center=${encodeURIComponent(state.windowCenter)}&window_width=${encodeURIComponent(state.windowWidth)}`;
}
...
function camOf(canvas, items) { return canvasCams.get(canvas) || camFor(sceneBounds(items), Math.max(1, canvas.clientWidth), Math.max(1, canvas.clientHeight)); }
function mapWorld(point, canvas) { if (point.world) return point.world; const cam = camOf(canvas, scenePlan().map((entry) => entry.item)); const selected = currentObject(); return unproject(point, cam, selected?.geometry ? centroidOf(selected) : null); }
...
function hitTest(point, canvas) { const all = scenePlan().map((entry) => entry.item); const cam = camOf(canvas, all); let hit = null; let best = Infinity; all.forEach((item) => { if (item.type === "volume") return; const projected = project(centroidOf(item), cam); const size = projectSize(item, cam); const radius = Math.max(20, size[0] * .5, size[1] * .5); const distance = Math.hypot(point.x - projected[0], point.y - projected[1]); const score = distance / radius; if (distance < radius && score < best) { best = score; hit = item; } }); return hit; }

function deselectObject() {
  const label = currentObject()?.label;
  state.highlights = state.highlights.filter((id) => id !== state.selectedId);
  state.selectedId = ""; state.isolate = false; state.showLinks = false; state.pathPoints = [];
  renderInspector(); renderRailObjects(); renderAnalysisPanels(); drawAll(); updateCrosshair();
  toast(label ? `${displayLabel(label)} deselected` : "Selection cleared");
}
function nearestSliceIndex(object) { const targetZ = object.geometry.centroid[2]; let nearestIndex = 0; let nearestDelta = Infinity; state.volume.instances.forEach((instance, index) => { const delta = Math.abs(Number(instance.z || index) - targetZ); if (delta < nearestDelta) { nearestDelta = delta; nearestIndex = index; } }); return nearestIndex; }
async function selectObject(id) { if (!objects().some((item) => item.id === id)) return; state.selectedId = id; state.isolate = false; state.pathPoints = []; const object = currentObject(); const group = activeAnatomyGroup(); if (group && !group.matches(object)) state.scenePreset = "all"; if (object?.geometry && state.volume?.instances?.length) { const nearestIndex = nearestSliceIndex(object); state.slice = nearestIndex; $("sliceInput").value = String(nearestIndex); await refreshSlices(); } renderInspector(); renderRailObjects(); renderAnalysisPanels(); drawAll(); updateCrosshair(); broadcastHologramSync(); toast(`${displayLabel(currentObject().label)} selected · linked views updated`); }
...
// Linked timelines are stored with study ids and compile times; show the study a person would recognise.
function timelineStudy(entry) { const study = state.studies.find((item) => item.id === entry.study_id); const stored = entry.label && entry.label !== entry.study_id ? entry.label : ""; return { title: shortStudyTitle(study?.description || stored) || "Earlier study", date: formatStudyDate(study?.study_date || (stored ? entry.date : "")) }; }
function renderTimeline() {
  const model = state.model; const timeline = model?.timeline || [];
  $("timelineRail").innerHTML = timeline.length ? timeline.map((entry, index) => `<div class="timeline-node ${index < timeline.length - 1 ? "prior" : ""}" style="left:${timeline.length === 1 ? 50 : (index / (timeline.length - 1)) * 100}%"><div class="timeline-node-label"><strong>${escapeHtml(timelineStudy(entry).title)}</strong>${escapeHtml(timelineStudy(entry).date)}</div></div>`).join("") : "";
  const hasPrior = Boolean(model?.metadata?.prior_model_id || (model?.temporal_links || []).length) && Boolean(state.priorModel);
  $("timelineStatus").textContent = hasPrior ? `${model.temporal_links?.length || 0} structures matched to the earlier study` : "No linked prior";
  ...
  $("temporalModeNote").textContent = hasPrior ? notes[state.temporalMode] : "No earlier study is linked, so only the current study can be shown.";
  const links = model?.temporal_links || [];
  if (!links.length) {
    const related = state.studies.filter((study) => study.id !== state.study?.id && study.patient_id && study.patient_id === state.study?.patient_id && study.status === "ready").sort((a, b) => String(b.study_date).localeCompare(String(a.study_date)))[0];
    const later = related && String(related.study_date) > String(state.study?.study_date);
    $("timelineDetail").innerHTML = `<div class="timeline-empty"><strong>Nothing to compare yet.</strong><p>${hasPrior ? "The earlier study is linked, but no object in it matched one in this study." : "Change is measured against an earlier study of the same patient, and none is linked to this one."}</p>${related ? `<button class="quiet-action" data-open-related="${escapeHtml(related.id)}">${later ? "Open the follow-up" : "Open the other study"} · ${escapeHtml((related.description || "").split(" (")[0])}</button>` : ""}</div>`;
    $("timelineDetail").querySelector("[data-open-related]")?.addEventListener("click", (event) => openStudy(event.currentTarget.dataset.openRelated).then(() => setMode("timeline")));
    return;
  }
  const changeLinks = links.filter((link) => link.type !== "same_as_prior");
  const rows = changeLinks.slice(0, 8).map((link) => { const object = (model.objects || []).find((item) => item.id === link.source_object_id) || (state.priorModel?.objects || []).find((item) => item.id === link.target_object_id); const changes = link.changes || {}; const details = [changes.volume_change_percent == null ? "" : `volume ${changes.volume_change_percent > 0 ? "+" : ""}${number(changes.volume_change_percent)}%`, changes.diameter_delta_mm == null ? "" : `diameter ${changes.diameter_delta_mm > 0 ? "+" : ""}${number(changes.diameter_delta_mm)} mm`, changes.surface_area_delta_mm2 == null ? "" : `surface ${changes.surface_area_delta_mm2 > 0 ? "+" : ""}${number(changes.surface_area_delta_mm2)} mm²`, changes.centroid_distance_mm == null ? "" : `centroid ${number(changes.centroid_distance_mm)} mm`].filter(Boolean).join(" · "); return `<button class="temporal-change-row" data-temporal-object="${escapeHtml(object?.id || link.source_object_id)}"><span class="temporal-change-type ${escapeHtml(link.type)}">${escapeHtml(humanStatus(link.type))}</span><strong>${escapeHtml(displayLabel(object?.label) || link.source_object_id)}</strong><small>${escapeHtml(details || "No numeric geometry delta")}</small><em>›</em></button>`; }).join("");
...
function renderRailObjects() {
  ...
  list.innerHTML = items.length ? items.map((item) => `<button class="rail-row ${item.id === state.selectedId ? "active" : ""} ${isHighlighted(item) ? "flagged" : ""}" data-rail-object="${escapeHtml(item.id)}"><i style="background:${typeColor(item)}"></i><span><strong>${escapeHtml(displayLabel(item.label))}</strong><small>${escapeHtml(item.type)}</small></span><em>${isHighlighted(item) ? "flagged" : "›"}</em></button>`).join("") : `<div class="rail-placeholder">Open a compiled model to browse objects.</div>`;
...
function renderAnalysisPanels() {
  ...
  evidence.innerHTML = sources.length ? sources.map((source) => `<button class="evidence-row" data-evidence-source="${escapeHtml(source.id)}"><span>${escapeHtml((source.type || "E").slice(0, 1).toUpperCase())}</span><div><strong>${escapeHtml(source.title || "Evidence")}</strong><small>${escapeHtml(source.date || "Undated")} · provenance attached</small></div><em>Open</em></button>`).join("") : `<div class="rail-placeholder">No evidence is bound to ${escapeHtml(displayLabel(object.label))}.</div>`;
...
// The cache only spares the background prefetch a second download. Opening a study always asks the server,
// because a model keeps its id while its content changes (context imports, reviews, recompiles).
async function loadModelCached(id, { fresh = false } = {}) { if (!id) return null; if (!fresh && modelCache.has(id)) return modelCache.get(id); const model = await api(`/api/patient-models/${encodeURIComponent(id)}`); modelCache.set(id, model); return model; }
...
async function loadStudies() { try { state.studies = await api("/api/studies"); state.offline = false; renderStudies(); if (!state.study && state.studies.length) { const ready = state.studies.find((study) => /follow-up/i.test(study.description || "") && study.status === "ready") || state.studies.find((study) => study.status === "ready") || state.studies[0]; await openStudy(ready.id); } else renderWorkspace(); } catch (error) { state.offline = true; renderWorkspace(); toast(`Workspace unavailable · ${error.message}`); } }
...
// The empty overlay covers three different situations. Each gets its own words and one primary action, so the
// big button never sits there doing nothing: no study yet, a study that still needs its model built, or no server.
function emptySituation() { return state.offline ? "offline" : state.study && !state.model ? "uncompiled" : "start"; }
function renderEmptyState() {
  const root = $("emptyState"); if (!root || state.demoSeeding) return; const situation = emptySituation();
  const copy = { start: ["PATIENT MODEL WORKSPACE", "Start with a study", "Explore the full workflow with included studies, or open your own DICOM study.", "Explore guided workspace"], uncompiled: ["STUDY INDEXED", "Build the patient model", `${state.study?.description || "This study"} is imported. Building its model turns the images into objects you can measure and compare.`, "Build patient model"], offline: ["NO CONNECTION", "Can’t reach the local server", "The workstation page loaded, but the Phasmed server is not answering. Start it, then try again.", "Try again"] }[situation];
  root.querySelector(".empty-kicker").textContent = copy[0]; root.querySelector("h1").textContent = copy[1]; root.querySelector("p").textContent = copy[2]; $("emptyDemo").textContent = copy[3]; $("emptyDemo").disabled = false;
  ["emptyImport", "emptyGateway"].forEach((id) => $(id).classList.toggle("hidden", situation !== "start")); root.querySelector("small").classList.toggle("hidden", situation !== "start");
}
function emptyPrimaryAction() { const situation = emptySituation(); if (situation === "uncompiled") return startCompile(state.study.id); if (situation === "offline") return loadStudies(); return seedDemoWorkspace(); }

function showDemoSeed(job, failed = false) {
  const button = $("emptyDemo"); const status = $("emptyDemoStatus"); if (!button || !status) return;
  state.demoSeeding = Boolean(job) && !failed;
...
async function openGateway() {
  showSheet("gatewaySheet"); $("gatewayCapabilities").innerHTML = `<span>Checking configured sources…</span>`; $("gatewayStudies").innerHTML = `<div class="rail-placeholder">Searching the image server…</div>`; $("gatewayStaged").innerHTML = `<div class="rail-placeholder">Loading staged studies…</div>`;
  try {
    const [capabilities, staged] = await Promise.all([api("/api/dicomweb/capabilities"), api("/api/dicomweb/cstore/studies")]);
    $("gatewayCapabilities").innerHTML = Object.entries(capabilities).filter(([key]) => key !== "c_store_receiver").map(([key, value]) => `<div><span>${escapeHtml(humanStatus(key))}</span><strong class="${value === "available" ? "available" : "unavailable"}">${escapeHtml(humanStatus(value))}</strong></div>`).join("");
...
async function modelTool(question) { const object = currentObject(); if (!object || !state.model) return "Open a patient model and select a structure first."; const lower = question.toLowerCase(); let tool = "get_object"; let argumentsValue = { object_id: object.id }; if (lower.includes("closest") || lower.includes("near")) tool = "get_neighbors"; else if (lower.includes("change") || lower.includes("prior") || lower.includes("history")) tool = "get_changes"; else if (lower.includes("evidence") || lower.includes("source") || lower.includes("context")) tool = "get_context"; const result = await api(`/api/patient-models/${encodeURIComponent(state.model.id)}/tools`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ tool, arguments: argumentsValue }) }); if (tool === "get_neighbors") { const first = result.results?.[0]; return first ? `The closest indexed structure is <b>${escapeHtml(displayLabel(first.label))}</b> at <b>${number(first.distance_mm)} mm</b>. This is a deterministic surface-distance query.` : `No neighboring geometry is available for <b>${escapeHtml(displayLabel(object.label))}</b>.`; } if (tool === "get_changes") { const link = result.links?.[0]; return link ? `<b>${escapeHtml(displayLabel(object.label))}</b> has a persisted <b>${escapeHtml(link.type)}</b> link. ${link.changes?.volume_change_percent == null ? "No numeric volume change is available." : `The calculated volume change is <b>${number(link.changes.volume_change_percent)}%</b>.`}` : `No prior link is available for <b>${escapeHtml(displayLabel(object.label))}</b>.`; } if (tool === "get_context") { return result.results?.length ? `<b>${result.results.length}</b> context bindings are attached to <b>${escapeHtml(displayLabel(object.label))}</b>. The source and review state are available in the evidence inspector.` : `No context is bound to <b>${escapeHtml(displayLabel(object.label))}</b>.`; } return `<b>${escapeHtml(displayLabel(object.label))}</b> is a ${escapeHtml(object.type)} object with persisted geometry and provenance.`; }
// ---- Chat assistant ---------------------------------------------------------
// The assistant reads a de-identified digest of the open PatientModel on the server and may ask the
// interface to flag or select objects. Everything it sends back is applied from a whitelist below.
const CHAT_VIEW_MODES = ["model", "slices", "timeline", "hologram"]; const CHAT_TEMPORAL_MODES = ["current", "prior", "overlay", "difference"];
async function chatCapability(force = false) { if (state.chat.capability && !force) return state.chat.capability; try { const health = await api("/api/health"); state.chat.capability = { configured: health.capabilities?.assistant === "configured", endpoint: health.capabilities?.assistant_endpoint || "", context: health.capabilities?.assistant_context || "" }; } catch { state.chat.capability = { configured: false, endpoint: "", context: "" }; } renderChatNote(); return state.chat.capability; }
function renderChatNote() { const note = $("chatNote"); if (!note) return; const cap = state.chat.capability; note.textContent = !cap ? "" : cap.configured ? `AI answers via ${cap.endpoint}. Sends a de-identified summary (labels, measurements${cap.context === "shared" ? ", report text" : ""}) — no images, names or IDs. Not a diagnosis.` : "No assistant key configured — using built-in keyword queries. Add PHASEMED_OPENAI_API_KEY to .env and restart."; }
function resetChat() { state.chat.abort?.abort(); state.chat = { ...state.chat, messages: [], busy: false, abort: null, modelVersion: null }; renderChat(); }
function chatObject(id) { return [...(state.model?.objects || []), ...(state.priorModel?.objects || [])].find((item) => item.id === id) || null; }
// Text is escaped first; only refs the server vouched for become chips. No markdown, links or images are ever rendered.
function chatHtml(text, refs = {}) { return escapeHtml(text).replace(/\[\[([OP]\d+)\]\]/g, (token, ref) => { const entry = refs[ref]; const object = entry && chatObject(entry.id); return object ? `<button type="button" class="chat-chip" data-chat-object="${escapeHtml(object.id)}"><i style="background:${meshColor(object)}"></i>${escapeHtml(entry.label || object.label)}</button>` : ""; }).replace(/\n{2,}/g, "</p><p>").replace(/\n/g, "<br />"); }
function renderChat() {
  const thread = $("chatThread"); if (!thread) return; const messages = state.chat.messages;
  thread.innerHTML = messages.length ? messages.map((message, index) => {
    if (message.role === "user") return `<div class="chat-message user"><p>${escapeHtml(message.text)}</p></div>`;
    const body = message.html != null ? `<p>${message.html}</p>` : message.text ? `<p>${chatHtml(message.text, message.refs)}</p>` : "";
    const status = message.pending ? `<div class="chat-status"><span></span>${escapeHtml(message.status || "Thinking")}</div>` : "";
    const flagged = (message.flagged || []).map(chatObject).filter(Boolean);
    const flags = flagged.length ? `<div class="chat-flags"><small>Flagged</small>${flagged.map((object) => `<button type="button" class="chat-chip flagged" data-chat-object="${escapeHtml(object.id)}"><i></i>${escapeHtml(displayLabel(object.label))}</button>`).join("")}<button type="button" class="chat-clear" data-chat-clear>Clear</button></div>` : "";
    const trace = (message.tools || []).length ? `<details class="chat-trace"><summary>How this was answered · ${message.tools.length} tool ${message.tools.length === 1 ? "call" : "calls"}</summary>${message.tools.map((entry) => `<div><b>${escapeHtml(entry.tool)}</b><code>${escapeHtml(JSON.stringify(entry.arguments))}</code><code>${escapeHtml(JSON.stringify(entry.result).slice(0, 700))}</code></div>`).join("")}</details>` : "";
    const error = message.error ? `<p class="chat-error">${escapeHtml(message.error)}</p>` : "";
    return `<div class="chat-message assistant" data-chat-index="${index}">${body}${status}${error}${flags}${trace}</div>`;
  }).join("") : `<div class="chat-empty"><strong>Ask about this model</strong><span>What changed, what is near a finding, how large a structure is. The assistant flags what it describes in the 3D view.</span></div>`;
  thread.querySelectorAll("[data-chat-object]").forEach((chip) => chip.addEventListener("click", () => selectObject(chip.dataset.chatObject)));
  thread.querySelectorAll("[data-chat-clear]").forEach((button) => button.addEventListener("click", () => { clearHighlights(); state.chat.messages.forEach((message) => { message.flagged = []; }); renderChat(); }));
  thread.scrollTop = thread.scrollHeight;
  const send = $("chatSend"); if (send) { send.textContent = state.chat.busy ? "■" : "→"; send.setAttribute("aria-label", state.chat.busy ? "Stop" : "Send"); send.classList.toggle("busy", state.chat.busy); }
  $("chatSuggestions")?.classList.toggle("hidden", messages.length > 0);
}
async function applyAssistantAction(action, turn) {
  const known = (ids) => (ids || []).filter((id) => typeof id === "string" && chatObject(id));
  if (action.action === "flag") { const ids = known(action.object_ids); if (!ids.length) return; state.highlights = turn.flaggedOnce ? [...new Set([...state.highlights, ...ids])] : ids; turn.flaggedOnce = true; turn.flagged = [...state.highlights]; if (!["model", "slices", "procedure", "hologram"].includes(state.mode)) setMode("model"); drawAll(); renderInspector(); renderRailObjects(); }
  else if (action.action === "select") { const [id] = known(action.object_ids); if (id && objects().some((item) => item.id === id)) await selectObject(id); }
  else if (action.action === "clear") { clearHighlights(); turn.flagged = []; turn.flaggedOnce = false; }
...
async function sendChat(question) {
  ...
  if (!state.model) { toast("Build this study's patient model first"); return false; }
  ...
  state.chat.messages.push({ role: "user", text }, turn); state.chat.busy = true; renderChat();
  try {
    if (!capability.configured) { turn.html = await modelTool(text); turn.local = true; }
    else {
      state.chat.abort = new AbortController();
      const response = await fetch(`/api/patient-models/${encodeURIComponent(state.model.id)}/assistant`, { method: "POST", signal: state.chat.abort.signal, headers: { "content-type": "application/json" }, body: JSON.stringify({ messages: [...history, { role: "user", content: text.slice(0, 4000) }], selected_object_id: state.selectedId, highlights: state.highlights.slice(0, 200), mode: state.mode, temporal_mode: state.temporalMode }) });
      if (response.status === 503) { await chatCapability(true); turn.html = await modelTool(text); turn.local = true; }
      else if (!response.ok) { let detail = response.statusText; try { detail = (await response.json()).detail || detail; } catch {} throw new Error(typeof detail === "string" ? detail : "The request was not accepted"); }
      else {
        const reader = response.body.getReader(); const decoder = new TextDecoder(); let buffer = "";
        const handle = async (event) => {
          if (event.type === "status") turn.status = event.text;
          else if (event.type === "delta") { turn.text += event.text; turn.status = "Writing"; }
          else if (event.type === "action") await applyAssistantAction(event, turn);
          else if (event.type === "done") { turn.text = event.text; turn.refs = event.refs || {}; turn.tools = event.tools || []; if (state.chat.modelVersion != null && state.chat.modelVersion !== event.model_version) state.chat.messages = state.chat.messages.slice(-2); state.chat.modelVersion = event.model_version; }
          else if (event.type === "error") turn.error = event.message || "The assistant could not answer.";
          renderChat();
        };
        for (;;) { const { value, done } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); let cut; while ((cut = buffer.indexOf("\n")) >= 0) { const line = buffer.slice(0, cut).trim(); buffer = buffer.slice(cut + 1); if (line) { let event = null; try { event = JSON.parse(line); } catch {} if (event) await handle(event); } } }
      }
    }
...
// In hologram mode the inspector (and its Chat tab) is hidden, so a spoken question is answered on the display itself.
function showHologramCaption(question, turn) {
  const caption = $("hologramCaption"); if (!caption || !turn) return;
  const answer = turn.html != null ? turn.html : escapeHtml(turn.error || turn.text || "").replace(/\[\[([OP]\d+)\]\]/g, (token, ref) => `<b>${escapeHtml(turn.refs?.[ref]?.label || "")}</b>`);
  caption.innerHTML = `<small>“${escapeHtml(question)}”</small><p>${answer}</p>`; caption.classList.remove("hidden");
  clearTimeout(state.captionTimer); state.captionTimer = setTimeout(() => caption.classList.add("hidden"), 22000);
}
...
function wireEvents() {
  ...
  $("contextImport").addEventListener("click", () => state.model ? $("contextInput").click() : toast("Build the patient model before adding evidence")); $("contextInput").addEventListener("change", (event) => importContextFile(event.target.files[0]));
  ...
  // Keys that work in every mode. Hologram keeps its own handler above; an open sheet or a focused field wins.
  const closeUtilityMenu = () => document.querySelector(".utility-menu")?.removeAttribute("open");
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") { if (document.querySelector(".sheet:not(.hidden)")) closeAllSheets(); else if (document.querySelector(".utility-menu[open]")) closeUtilityMenu(); else if (state.mode === "hologram") { stopVoiceCapture(); setMode("model"); } return; }
    if (event.metaKey || event.ctrlKey || event.altKey || ["INPUT", "TEXTAREA", "SELECT"].includes(document.activeElement?.tagName) || document.activeElement?.isContentEditable) return;
    if (event.key === "?") { event.preventDefault(); closeUtilityMenu(); showSheet("shortcutsSheet"); return; }
    if (document.querySelector(".sheet:not(.hidden)") || !state.model) return;
    const modes = ["model", "slices", "timeline", "context", "procedure", "hologram"];
    if (/^[1-6]$/.test(event.key)) { event.preventDefault(); document.querySelector(`.mode-item[data-mode="${modes[Number(event.key) - 1]}"]`)?.click(); }
    else if (event.key === "/" && state.mode !== "hologram") { event.preventDefault(); openChat(); }
    else if (state.mode === "model" && document.activeElement !== $("sceneCanvas")) {
      if (["+", "="].includes(event.key)) { event.preventDefault(); zoomScene(1.2); }
      else if (["-", "_"].includes(event.key)) { event.preventDefault(); zoomScene(.8); }
      else if (event.key === "0") { event.preventDefault(); fitScene(); }
    }
  });
  document.addEventListener("click", (event) => { const menu = document.querySelector(".utility-menu[open]"); if (menu && (!menu.contains(event.target) || event.target.closest(".utility-item"))) closeUtilityMenu(); });
  $("shortcutsButton").addEventListener("click", () => showSheet("shortcutsSheet"));
  $("sceneAsk").addEventListener("click", () => openChat());
  ...
  // Orbit, pan and zoom for the canvases that own their camera (review overview, procedure).
  Object.keys(canvasViews).forEach((id) => {
    const canvas = $(id); if (!canvas) return; const view = canvasViews[id]; let drag = null;
    canvas.style.cursor = "grab"; canvas.style.touchAction = "none";
    canvas.addEventListener("contextmenu", (event) => event.preventDefault());
    canvas.addEventListener("pointerdown", (event) => { const pan = event.shiftKey || event.button === 1 || event.button === 2; drag = { pan, x: event.clientX, y: event.clientY, ...view, moved: false }; view.suppressClick = false; canvas.setPointerCapture(event.pointerId); canvas.style.cursor = pan ? "move" : "grabbing"; });
    canvas.addEventListener("pointermove", (event) => { if (!drag) return; const dx = event.clientX - drag.x; const dy = event.clientY - drag.y; drag.moved ||= Math.hypot(dx, dy) > 3; if (drag.pan) { view.panX = drag.panX + dx; view.panY = drag.panY + dy; } else { view.yaw = drag.yaw + dx * .008; view.pitch = clamp(drag.pitch + dy * .008, -Math.PI / 2, Math.PI / 2); } requestDraw(); });
    const end = () => { if (!drag) return; view.suppressClick = drag.moved; drag = null; canvas.style.cursor = "grab"; };
    canvas.addEventListener("pointerup", end); canvas.addEventListener("pointercancel", end);
    canvas.addEventListener("wheel", (event) => { if (!state.model) return; event.preventDefault(); view.zoom = clamp(view.zoom * (event.deltaY > 0 ? .88 : 1.14), .3, 8); requestDraw(); }, { passive: false });
    canvas.addEventListener("dblclick", () => { if (state.pathMode) return; Object.assign(view, newView()); requestDraw(); });
  });
  $("procedureCanvas").addEventListener("click", (event) => { if (!state.pathMode) return; if (canvasViews.procedureCanvas.suppressClick) { canvasViews.procedureCanvas.suppressClick = false; return; } const canvas = $("procedureCanvas"); const point = pointFromEvent(event, canvas); point.world = mapWorld(point, canvas); state.pathPoints.push(point); if (state.pathPoints.length === 2) finishPath(state.pathPoints[0], state.pathPoints[1], canvas); drawAll(); });
  ...
  ["capabilityInfoLibrary", "capabilityInfoInspector"].forEach((id) => $(id).addEventListener("click", () => showSheet("capabilitySheet"))); $("contextAskModel").addEventListener("click", () => openChat()); $("chatForm").addEventListener("submit", (event) => { event.preventDefault(); if (state.chat.busy) { state.chat.abort?.abort(); return; } const question = $("chatInput").value; $("chatInput").value = ""; sendChat(question); }); document.querySelectorAll("[data-chat-query]").forEach((button) => button.addEventListener("click", () => sendChat(button.dataset.chatQuery))); document.querySelector('[data-analysis-tab="chat"]').addEventListener("click", () => { chatCapability(true); renderChat(); }); $("contextSearchForm").addEventListener("submit", async (event) => { event.preventDefault(); if (!state.model) return; state.contextQuery = $("contextQueryInput").value.trim(); try { const result = await api(`/api/patient-models/${encodeURIComponent(state.model.id)}/context-query`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ query: state.contextQuery }) }); renderContextView(result.results); } catch (error) { toast(`Context search failed · ${error.message}`); } });
