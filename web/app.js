...
function renderInspector() {
  const object = currentObject(); $("inspectorTitle").textContent = displayLabel(object?.label) || "Nothing selected"; $("inspectorSubtitle").textContent = object?.id || ""; $("inspectorSubtitle").title = object?.id || "";
...
const canvasViews = { mprModelCanvas: newView() };
...
function voiceSummary(result) { const names = (result.labels || []).map(displayLabel); return names.length <= 3 ? names.join(", ") : `${names[0]}, ${names[1]} and ${names.length - 2} more`; }
function applyVoiceCommand(result) {
  ...
  else if (result?.targets?.length) { state.highlights = result.targets; toast(`Highlighted ${voiceSummary(result)}`); }
  else { state.highlights = []; toast(`Heard “${state.voiceTranscript}” · no matching structure`); }
  setVoiceState("idle");
  drawAll(); renderInspector(); renderRailObjects();
}
function drawAll() { if (state.model && state.mode === "model") drawScene(); if (state.model && state.mode === "slices") drawSceneOn($("mprModelCanvas"), true); if (state.model && state.mode === "hologram") drawHologram(); }
...
function renderRailObjects() {
  ...
  list.innerHTML = items.length ? items.map((item) => `<button class="rail-row ${item.id === state.selectedId ? "active" : ""}" data-rail-object="${escapeHtml(item.id)}"><i style="background:${typeColor(item)}"></i><span><strong>${escapeHtml(displayLabel(item.label))}</strong><small>${escapeHtml(item.type)}</small></span><em>›</em></button>`).join("") : `<div class="rail-placeholder">Open a compiled model to browse objects.</div>`;
...
function renderProcedureResult(result) { $("pathReadout").classList.toggle("hidden", !result); if (!result) { $("procedureResult").innerHTML = ""; return; } const clearance = (result.clearance || []).filter((item) => item.clearance_mm != null).sort((a, b) => a.clearance_mm - b.clearance_mm).slice(0, 4); $("procedureResult").innerHTML = `<h3>Proposed path</h3><div class="path-stat"><span>Length</span><strong>${number(result.length_mm)} mm</strong></div><div class="path-stat"><span>Intersections</span><strong>${result.intersections?.length || 0} objects</strong></div>${clearance.map((item) => `<div class="path-stat"><span>${escapeHtml(displayLabel(item.label))}</span><strong>${number(item.clearance_mm)} mm clear</strong></div>`).join("")}<p class="path-method">Method · ${escapeHtml(result.method || "segment geometry")}</p>`; }
function activateTool(tool) { state.activeTool = tool; document.querySelectorAll("[data-tool]").forEach((item) => item.classList.toggle("active", item.dataset.tool === tool)); const cursor = tool === "rotate" ? "grab" : tool === "pan" ? "move" : tool === "measure" || tool === "path" ? "crosshair" : tool === "zoom" ? "zoom-in" : "default"; $("sceneCanvas").style.cursor = cursor; }
// Ending path mode keeps the drawn line so it stays on screen beside its result; clearPath removes both.
function leavePathMode() { state.pathMode = false; if (state.activeTool === "path") activateTool("rotate"); }
function clearPath() { leavePathMode(); state.pathPoints = []; renderProcedureResult(null); drawAll(); }
function startPath() { if (!state.model) { toast("Compile a PatientModel before drawing a path"); return; } if (state.mode !== "model") setMode("model"); clearPath(); state.pathMode = true; activateTool("path"); toast("Click an entry point, then a target, on the model"); drawAll(); }
...
function wireEvents() {
  ...
  document.querySelectorAll("[data-tool]").forEach((button) => button.addEventListener("click", () => { const tool = button.dataset.tool; if (tool === "path") { if (state.pathMode) { clearPath(); toast("Path drawing canceled"); } else startPath(); return; } if (tool === "measure" && state.activeTool === "measure") { activateTool("rotate"); toast("Measure tool closed"); return; } activateTool(tool); toast(`${tool[0].toUpperCase() + tool.slice(1)} tool active`); }));
  ...
  $("clearPath").addEventListener("click", () => { clearPath(); toast("Path cleared"); });
  ...
  // Orbit, pan and zoom for the canvases that own their camera (the review overview).
  Object.keys(canvasViews).forEach((id) => {
    const canvas = $(id); const view = canvasViews[id]; let drag = null;
